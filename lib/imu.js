  ...
  LSM303C: {
    ...
    REGISTER: {
      value: {
        ...
        //////////////////////
        MAG_CTRL_REG: [0x20, 0x21, 0x22, 0x23, 0x24],
        //////////////////////
        MAG_STATUS_REG: 0x27,
        //////////////////////
        MAG_OUTX_L: 0x28,
    ...
    initialize: {
      value: function(board, opts) {
        ...
        // Setup
        [0,1,4,0,3,2]
          .forEach(n => io.i2cWrite(address, this.REGISTER.MAG_CTRL_REG[n], this.REGISTER.MAG_SETUP[n]));

        var readMag = () => new Promise(resolve => {
          io.i2cReadOnce(address, this.REGISTER.MAG_OUTX_L, 6, data => {
            computed.magnetometer = {
              x: int16(data[1], data[0]) * SENSITIVITY,
              y: int16(data[3], data[2]) * SENSITIVITY,
              z: int16(data[5], data[4]) * SENSITIVITY
            };

            if (data.length > 5) {
              computed.temperature = int16(data[7], data[6]);
            }
        ...
        var readTemp = () => new Promise(resolve => {
          io.i2cReadOnce(address, this.REGISTER.MAG_TEMP_OUT_L, 2, data => {
            computed.temperature = int16(data[1], data[0]);
            resolve();
          });
        });

        readCycle = () => setTimeout(() => {
          io.i2cReadOnce(address, this.REGISTER.MAG_STATUS_REG, 1, data => {
            //console.log({MAG_STATUS_REG: data || 'NOTHING!'});
            //if (!data) {
              // MAG_STATUS_REG returns 0x00 when values have changed
              readMag()
              .then(readTemp)
              .then(() => {
                this.emit("data", computed);
...
var Controllers = {
  ...
  LSM303C: {
    ...
    components: {
      value: [MAGNETOMETER, THERMOMETER]
    },
  },
};
