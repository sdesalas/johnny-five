...
var Drivers = {
  ...
  /**
   * LSM303C: 6Dof 3-Axis Magnetometer & Accelerometer (cheapie clone)
   * 
  ...
  LSM303C: {
    ...
    COMMAND: {
      value: {
        SETUP: [0xD8, 0x60, 0x40, 0xD8, 0x8, 0x00],
      }
    },
    REGISTER: {
      value: {
    ...
    initialize: {
      value: function(board, opts) {
        var SENSITIVITY = 0.00048828125; // LSB/Ga
        ...
        this.REGISTER.MAG_CTRL_SEQ
          .forEach(function (reg, i) { 
            io.i2cWrite(address, reg, this.COMMAND.SETUP[i]);
          }.bind(this));

        // Read Magnetometer
        var readMag = function(resolve) {
          io.i2cReadOnce(address, this.REGISTER.MAG_OUTX_L, 6, function(data) {
            computed.magnetometer = {
        ...
        // Read temperature
        var readTemp = function(resolve) {
          io.i2cReadOnce(address, this.REGISTER.MAG_TEMP_OUT_L, 2, function(data) {
        ...
        var readCycle = function() { 
          Promise.all([
            new Promise(readMag),
            new Promise(readTemp)
          ])
          .then(function() {
            this.emit("data", computed);
            setTimeout(readCycle, 40);
          }.bind(this));
        }.bind(this);

        // Kick off
        setTimeout(readCycle, 200);
      },
    },
    identifier: {
      value: function(opts) {
