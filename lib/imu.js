...
var Drivers = {
  ...
  LSM303C: {
    ...
    REGISTER: {
      value: {
        MAG_SETUP: [0xD8, 0x60, 0x00, 0x8, 0x40],
        //////////////////////
        MAG_CTRL_REG1: 0x20,
        //////////////////////
        MAG_STATUS_REG: 0x27,
    ...
    initialize: {
      value: function(board, opts) {
        var SENSITIVITY_MAG = 0.00048828125;
        var SENSITIVITY_ACC = 0.06103515625;
        var io = board.io;
        var address = opts.address || this.ADDRESSES[0];
        ...
        io.i2cConfig(opts);
        io.i2cWrite(address, this.REGISTER.MAG_CTRL_REG1, this.REGISTER.MAG_SETUP);
        io.i2cWrite(address, this.REGISTER.ACC_SETUP_REG[0], this.REGISTER.ACC_SETUP_REG[0]);
        io.i2cWrite(address, this.REGISTER.ACC_SETUP_REG[1], this.REGISTER.ACC_SETUP_REG[1]);
        setTimeout(() => {
          io.i2cRead(address, this.REGISTER.MAG_OUTX_L, 6, function(data) {
  
            computed.magnetometer = {
              x: int16(data[0], data[1]) * SENSITIVITY_MAG,
              y: int16(data[2], data[3]) * SENSITIVITY_MAG,
              z: int16(data[4], data[5]) * SENSITIVITY_MAG
            };
            //console.log('MAG Data returned from LSM303C...', data, computed);
  
            this.emit("data", computed);
          }.bind(this));
          io.i2cRead(address, this.REGISTER.ACC_OUTX_L, 6, function(data) {
            
            computed.accelerometer = {
              x: int16(data[0], data[1]) * SENSITIVITY_ACC,
              y: int16(data[2], data[3]) * SENSITIVITY_ACC,
              z: int16(data[4], data[5]) * SENSITIVITY_ACC
            };
            //console.log('ACC Data returned from LSM303C...', data, computed);
...
var Controllers = {
  ...
  LSM303C: {
    ...
    components: {
      value: [MAGNETOMETER, ACCELEROMETER]
    },
  },
};
