...
var Controllers = {
  ...
  LSM303C: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        var SENSITIVITY = 0.06103515625; // LSB/mg

        var address = opts.address || this.ADDRESSES[0];
        var state = priv.get(this);
        var dataPoints = { x: 0.0, y: 0.0, z: 0.0 };
        var io = this.io;

        io.i2cConfig(Object.assign({}, opts, { address }));
        ...
        // Define steps to read every cycle
        var readCycle = function() {
          io.i2cReadOnce(address, this.REGISTER.OUTX_L, 6, function(data) {
            
            dataPoints.x = int16(data[1], data[0]) * SENSITIVITY;
            dataPoints.y = int16(data[3], data[2]) * SENSITIVITY;
            dataPoints.z = int16(data[5], data[4]) * SENSITIVITY;
